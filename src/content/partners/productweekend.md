---
name: Product Weekend
kind: partner
order: 4
tagline: Exclusive 10% discount on a weekend of fun with other PMs.
hero:
  title: Product Weekend
  subtitle: Exclusive 10% discount on a weekend of fun with other PMs.
offer:
  headline: An exclusive 🚨 10% discount on all Product Circle events
  body: ""
  code: PWIMPOSTOR
  ctaLabel: Grab your spot!
  ctaUrl: https://www.productcircle.co/
seo:
  ogImage: /img/og/productweekend.webp
  title: Product Weekend 💛 The Impostor PM
  description: >-
    Impostor PM members get 10% off all Product Circle events.
---

**Spend a weekend at the office and have a great time**

It sounds odd, but it's actually what our participants and speakers have been saying.
